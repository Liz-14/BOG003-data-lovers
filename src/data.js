...
export const check = (pokeData) => {
  ...
  // copia de la data principal, elimina elemntos vacios
  const pokeDataInfo = pokeData.filter(el => el !== '')
  // Retorna los datos que se usaran paa crear la galeria y la info de cada card
  return { pokeData, pokeDataInfo }
}
...
// Recorre todo el array hasta encontrar el elemento que cumple la condicion
export const filterData = (data, condition) => {
  const newData = data.filter(el => el.name === condition)
  return newData
}

export const computeStats = (data) => {
  const basicStats = data.map(item => {
    const attack = Number(item.stats['base-attack'])
    const defense = Number(item.stats['base-defense'])
    const stamina = Number(item.stats['base-stamina'])
    return [attack, defense, stamina]
  })

  const sum = basicStats.map(item => {
    return item.reduce((a, b) => a + b, 0)
  })

  const average = sum.map(item => {
    return item / 3
  })
